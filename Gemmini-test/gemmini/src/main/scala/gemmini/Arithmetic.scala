...
// Added Complex data type - James add
class Complex(val w: Int) extends Bundle{
  val real = SInt(w.W) // SignedInteger
  val imag = SInt(w.W)
}
  ...
  implicit object ComplexArithmetic extends Arithmetic[Complex] {
    override implicit def cast(self: Complex) = new ArithmeticOps(self) {
      ...
      override def *(other: Complex): Complex = {
        ...
        // Sum = (a + bi)(c + di) = (ac -bd) + (ad + bc)i
        Complex(w,
          self.real * other.real - self.imag * other.imag, // real part
          self.real * other.imag + self.imag * other.real  // imag part
        )
      }
      // Complex addition
      override def +(other: Complex): Complex = {
        // Sum = (a + bi) + (c + di) = (a + c) + (b + d)i
        Complex(self.w max other.w,
          self.real + other.real, // real part
          self.imag + other.imag  // imag part
        )
      }
      // Complex subsitution
      override def -(other: Complex): Complex = {
      ...
      // Complex Multiply Accumulated (MAC)
      override def mac(m1: Complex, m2: Complex): Complex = {
        // Sum = Sum + m1 * m2
        self + m1 * m2 // Multiply Accumulate
      }
      // Complex value is zero
      override def zero = Complex(self.w, 0.S, 0.S)
      // Complex identity operation
      override def identity: Complex = self
      // Follow the bitwidth size of other
      ...
      override def minimum : Complex = self
    }
  }
  // End Add
