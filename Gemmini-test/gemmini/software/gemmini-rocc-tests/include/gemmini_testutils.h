...
static void cmplx_mult(elem_z A[DIM][DIM], elem_z B[DIM][DIM], elem_z Out[DIM][DIM]){

  for (size_t r = 0; r < DIM; r++){
    for (size_t c = 0; c < DIM; c++) {
      for (size_t k = 0; k < DIM; k++){
        Out[r][c].real += A[r][k].real * B[k][c].real - A[r][k].imag * B[k][c].imag;
        Out[r][c].imag += A[r][k].real * B[k][c].imag + A[r][k].imag * B[k][c].real;
      }
    }
  }
}
...
static void printMatrix(elem_z m[DIM][DIM]) {
...
// James add
static void printComplexMat(elem_z m[DIM][DIM]) {
  for (size_t i = 0; i < DIM; i++){
    for (size_t j = 0; j < DIM; j++)
#ifndef ELEM_T_IS_FLOAT
      printf("(%d+%di)", m[i][j].real, m[i][j].imag);
#else
      printf("%x ", elem_t_to_elem_t_bits(m[i][j]));
#endif
    printf("\n");
